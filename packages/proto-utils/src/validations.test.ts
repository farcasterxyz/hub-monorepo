...
describe('validateFid', () => {
  ...
  test('fails with 0', () => {
    expect(validations.validateFid(0)._unsafeUnwrapErr()).toEqual(
...
describe('validateCastId', () => {
  ...
  test('fails when both fid and tsHash are invalid', async () => {
    const castId = await Factories.CastId.build({ fid: -10, hash: new Uint8Array() });
    expect(validations.validateCastId(castId)._unsafeUnwrapErr()).toEqual(
...
describe('validateCastAddBody', () => {
  ...
  describe('fails', () => {
    ...
    test('when parent fid is missing', async () => {
      body = await Factories.CastAddBody.build({
        parentCastId: Factories.CastId.build({ fid: 0 }),
      });
      hubErrorMessage = 'fid is missing';
    });

    test('when parent tsHash is missing', async () => {
      body = await Factories.CastAddBody.build({ parentCastId: Factories.CastId.build({ hash: new Uint8Array() }) });
...
describe('validateCastRemoveBody', () => {
  ...
  test('fails when tsHash is missing', async () => {
    const body = await Factories.CastRemoveBody.build({ targetHash: new Uint8Array() });
  ...
  describe('fails', () => {
    ...
    test('when cast fid is missing', async () => {
      body = await Factories.ReactionBody.build({
        targetCastId: Factories.CastId.build({ fid: 0 }),
      });
      hubErrorMessage = 'fid is missing';
    });
    ...
    test('when cast hash is missing', async () => {
      body = await Factories.ReactionBody.build({
        targetCastId: Factories.CastId.build({ hash: new Uint8Array() }),
      });
      hubErrorMessage = 'tsHash is missing';
    });
  });
...
describe('validateVerificationAddEthAddressBody', () => {
  ...
  describe('fails', () => {
    ...
    test('with missing eth address', async () => {
      body = await Factories.VerificationAddEthAddressBody.create({ address: new Uint8Array() });
    ...
    test('with missing block hash', async () => {
      body = await Factories.VerificationAddEthAddressBody.create({ blockHash: new Uint8Array() });
...
describe('validateVerificationAddEthAddressSignature', () => {
  test('succeeds', async () => {
    ...
    const body = await Factories.VerificationAddEthAddressBody.build({}, { transient: { fid, network } });
  ...
  test('fails with invalid eth signature', async () => {
    ...
    const bodyT = new protobufs.VerificationAddEthAddressBodyT(
    ...
    const body = protobufs.VerificationAddEthAddressBody.getRootAsVerificationAddEthAddressBody(
...
describe('validateVerificationRemoveBody', () => {
  ...
  describe('fails', () => {
    ...
    test('when address is missing', async () => {
      body = await Factories.VerificationRemoveBody.build({
        address: [],
      });
      hubErrorMessage = 'address is missing';
    });
...
describe('validateSignerBody', () => {
  test('succeeds', async () => {
    const body = await Factories.SignerBody.build();
  ...
  describe('fails', () => {
    ...
    test('when signer is missing', async () => {
      body = await Factories.SignerBody.build({
        signer: [],
      });
...
describe('validateAmpBody', () => {
  test('succeeds', async () => {
    const body = await Factories.AmpBody.build();
  ...
  describe('fails', () => {
    ...
    test('when user fid is missing', async () => {
      body = await Factories.AmpBody.build({
        user: new protobufs.UserIdT([]),
      });
      hubErrorMessage = 'fid is missing';
    });
    ...
    test('with invalid user fid', async () => {
      body = await Factories.AmpBody.build({
        user: new protobufs.UserIdT(Array.from(Factories.Bytes.build({}, { transient: { length: 33 } }))),
      });
...
describe('validateUserDataAddBody', () => {
  test('succeeds', async () => {
    const body = await Factories.UserDataBody.build();
  ...
  describe('fails', () => {
    ...
    test('when pfp > 256', async () => {
      body = await Factories.UserDataBody.build({
