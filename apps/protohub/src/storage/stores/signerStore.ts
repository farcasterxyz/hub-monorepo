...
class SignerStore extends SequentialMergeStore {
   ...
   */
  static signerAddsKey(fid: number, signerPubKey?: Uint8Array): Buffer {
   ...
   * @returns RocksDB key of the form <RootPrefix>:<fid>:<UserPostfix>:<signerPubKey?>
   */
  static signerRemovesKey(fid: number, signerPubKey?: Uint8Array): Buffer {
    return Buffer.concat([
      makeUserKey(fid),
  ...
  async getCustodyEvent(fid: number): Promise<protobufs.IdRegistryEvent> {
    return protobufs.IdRegistryEvent.get(this._db, fid);
  }

  /** Returns the custody address that currently owns an fid */
  async getCustodyAddress(fid: number): Promise<Uint8Array> {
   ...
   */
  async getSignerAdd(fid: number, signerPubKey: Uint8Array): Promise<types.SignerAddModel> {
    const messageTsHash = await this._db.get(SignerStore.signerAddsKey(fid, signerPubKey));

    return MessageModel.get<types.SignerAddModel>(this._db, fid, UserPostfix.SignerMessage, messageTsHash);
  }
   ...
   */
  async getSignerRemove(fid: number, signer: Uint8Array): Promise<types.SignerRemoveModel> {
    const messageTsHash = await this._db.get(SignerStore.signerRemovesKey(fid, signer));
    return MessageModel.get<types.SignerRemoveModel>(this._db, fid, UserPostfix.SignerMessage, messageTsHash);
  }
   ...
   */
  async getSignerAddsByUser(fid: number): Promise<types.SignerAddModel[]> {
    ...
    return MessageModel.getManyByUser<types.SignerAddModel>(this._db, fid, UserPostfix.SignerMessage, messageKeys);
  }
   ...
   * @returns the SignerRemove message if it exists, throws HubError otherwise
   */
  async getSignerRemovesByUser(fid: number): Promise<types.SignerRemoveModel[]> {
    ...
    return MessageModel.getManyByUser<types.SignerRemoveModel>(this._db, fid, UserPostfix.SignerMessage, messageKeys);
  }
  ...
  async mergeIdRegistryEvent(event: protobufs.IdRegistryEvent): Promise<void> {
    ...
    protobufs.IdRegistryEvent.putTransaction(txn, event);
