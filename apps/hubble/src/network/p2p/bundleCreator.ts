import { HubResult, Message, MessageBundle } from "@farcaster/hub-nodejs";
...
export class BundleCreator {
  ...
  async broadcastBundle(): Promise<void> {
    if (this._currentBundle.length > 0) {
      ...
      const allHashes = Buffer.concat(bundle.map((message) => message.hash));
  ...
  async gossipMessage(message: Message): Promise<GossipPublishResult> {
    ...
    if (this._bundleIncomingMessages) {
      ...
      if (this._currentBundle.length >= MAX_BUNDLE_SIZE) {
        await this.broadcastBundle();
        this._currentBundle = [];
      }

      return { bundled: true, publishResults: [] };
    } else {
