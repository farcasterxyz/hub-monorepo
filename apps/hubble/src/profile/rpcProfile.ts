import {
  ...
  export async function profileRPCServer(addressString: string, useInsecure: boolean) {
	...
	// Initialize the appropriate rpcClient 
	const rpcClient = useInsecure
	  ? getInsecureHubRpcClient(addressString)
	  : getSSLHubRpcClient(addressString);
  ...
  async function profileSubmitMessages(
	...
	// Set metadata based on username/password or use default
	let metadata = username && typeof username !== "string" ? username : getAuthMetadata(username, password);
	...
	if (!idResult.isOk()) {
	  throw new Error(`Failed to submit custody event for fid ${fid}: ${idResult.error}`);
	}
	...
	if (!rentResult.isOk()) {
	  throw new Error(`Failed to submit rent event for fid ${fid}: ${rentResult.error}. NOTE: RPC profile only works on devnet`);
	}
	...
	if (!result.isOk()) {
	  throw new Error(`Failed to submit signer add message for fid ${fid}: ${result.error}`);
	}
	...
	if (!castAddResult.isOk()) {
	  throw new Error(`Failed to submit cast add message for fid ${fid}: ${castAddResult.error}`);
	}
	...
	if (!reactionAddResult.isOk()) {
	  throw new Error(`Failed to submit reaction add message for fid ${fid}: ${reactionAddResult.error}`);
	}
