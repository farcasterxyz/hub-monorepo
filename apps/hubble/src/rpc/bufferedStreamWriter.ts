 ...
 * or if the cache exceeds 1000 messages, the stream will be destroyed.
 */
export class BufferedDuplexStreamReaderWriter<T, R> {
  private streamIsBackedUp = false;
  private stream: ServerDuplexStream<T, R>;
  // biome-ignore lint/suspicious/noExplicitAny: legacy code, avoid using ignore for new code
  private dataWaitingForDrain: any[] = [];
  public closed = false;

  constructor(stream: ServerDuplexStream<T, R>) {
    this.stream = stream;
  }
