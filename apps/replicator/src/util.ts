...
export function convertProtobufMessageBodyToJson(message: Message): MessageBodyJson {
  ...
  switch (message.data?.type) {
    ...
    case MessageType.VERIFICATION_ADD_ADDRESS: {
      ...
      const { address, addressVerificationSignature, blockHash, protocol } = message.data.verificationAddAddressBody;
      switch (protocol) {
        case Protocol.ETHEREUM:
          return {
            address: bytesToHex(address),
            addressVerificationSignature: bytesToHex(addressVerificationSignature),
        ...
        case Protocol.SOLANA:
          return {
            ...
            addressVerificationSignature: new TextDecoder().decode(addressVerificationSignature),
