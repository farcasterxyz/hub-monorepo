import { FarcasterNetwork, MessageData } from '@farcaster/flatbuffers';
...
import { UserInfo } from '~/test/perf/setup';
...
export const makeBasicScenario = async (
  ...
  const casts = await makeMessages(userInfos, config, MockFCEvent.Cast, network);
  messages = messages.concat(casts);
  const amps = await makeMessages(userInfos, config, MockFCEvent.Amp, network);
  messages = messages.concat(amps);
  const reactions = await makeMessages(userInfos, config, MockFCEvent.Reaction, network);
