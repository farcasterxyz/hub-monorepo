...
// sets up hubs
const userInfos = await setupNetwork(rpcClients, {
  users: cliOptions['users'],
  mode: SetupMode.RANDOM_SINGLE,
  network: cliOptions['network'],
});
// creates scenario data
const scenario = await makeBasicScenario(rpcClients[0] as HubRpcClient, userInfos, cliOptions['network']);
