...
class BasicSimulator extends Simulator {
  ...
  async generateRandomVerification(client: Client): Promise<FC.VerificationAdd> {
    ...
    return client.makeVerificationAdd(ethAddress, claimToSign, externalSignature);
  }
  ...
  async generateMessages(client: Client) {
    ...
    const vAdd1 = await this.generateRandomVerification(client);
    const vAdd2 = await this.generateRandomVerification(client);
    const vRem1 = await client.makeVerificationRemove(vAdd1.data.body.claimHash);
    return [cs1, cs2, cs3, cd1, cs4, ra1, ru1, vAdd1, vAdd2, vRem1];
  }
}
